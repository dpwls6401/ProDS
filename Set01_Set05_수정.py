...
data4=pd.read_csv('/Users/yejin/ProDS/Dataset/Dataset_04.csv')
data4.columns
...
temp = q2[q2.SUBJECT == 'BEEF'] #육류 종류별로 들어가도록
...
# (3) 반복 적용
from scipy.stats import ttest_rel #메모리 활용도 우수
#import scipy.stats as st #호출하기 용이(함수목록 show)
#st.ttest

q2_out = []
for i in sub_list:
    temp = q2[q2.SUBJECT == i] #육류 종류별로 들어가도록 필터링!!
    temp2 = pd.pivot_table(temp, index='TIME',
                          columns='LOCATION',
                          values='Value',
                          aggfunc='mean').dropna()
...
# 마지막에 일괄적으로 데이터 프레임 활용해서 변경
q3_out = pd.DataFrame(q3_out, ['sub', 'r2', 'mape'])
...
# [참고]
q3['TIME'].shape
# lm = LinearRegression().fit(q3['TIME'], q3['Value']) ==> 차원 안 맞는 문제
lm = LinearRegression().fit(q3['TIME'].values.reshape(-1, 1), q3['Value'])
lm = LinearRegression().fit(q3[['TIME']], q3['Value'])


# [참고] 육류별로 별도 모델 생성 후 필요 시 모델 재사
# globals()['변수명'] = 모델 => eval('문자열') -> 문자열을 Object로 인식
# locals()
...
data5 = pd.read_csv('/Users/yejin/ProDS/Dataset/Dataset_05.csv', na_values=['?', ' ', ''])
data5.columns
...
# 답 : 0.0031, 귀무가설 기각(성별이 세분화에 영향 미침; p값이 0.05보다 작음)
